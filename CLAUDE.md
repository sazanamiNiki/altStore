# altStore-jp - AltStore PAL日本語ガイド & レトロゲーム×合法プレイ情報サイト

iPhoneのサイドローディング情報を日本語で提供するWebサイト。**AltStore PAL**（日本で無料利用可能な代替マーケットプレイス）を中心に、**Deltaエミュレータ+GB Operatorによる合法レトロゲームプレイ**を最大の差別化ポイントとして展開。

## 概要

- **目的**: AltStore PALの日本語ガイドと、レトロゲームの合法的な楽しみ方を提供
- **対象ユーザー**: iPhoneでレトロゲームを遊びたい人、AltStore PALを使いたい日本のiPhoneユーザー
- **差別化ポイント（他サイトにない独自コンテンツ）**:
  - **Delta+GB Operator+著作権法の統合解説** — カートリッジ→ROM吸い出し→Delta→iPhoneのフルフローを法的根拠付きで解説する日本語サイトは他にない
  - **スマホ新法（MSCA）施行後の実態レポート** — 法律解説だけでなく「今できること/まだできないこと」の具体情報
  - **エミュレータの法的分析** — Switch系エミュ壊滅の経緯、TPMの技術的解説
- **主なコンテンツ**:
  - AltStore PAL/Classic/SideStoreの選び方・セットアップガイド
  - Deltaエミュレータの導入・活用（無料ゲーム、GB Operator連携）
  - 法的・セキュリティ面での安全性情報（著作権法第30条、不正競争防止法）
  - スマホ新法の解説と施行後の最新動向

## SEO戦略

- **IndexNow**: デプロイ後に `npm run indexnow` でBing/Yandex等に即時通知
- **サイトマップ絞り込み**: 全ページではなく主要12ページのみサイトマップに含める（`astro.config.mjs` の `PRIORITY_PAGES`）
- **Accordion内の重要情報は常時表示**: Googleは非表示コンテンツを低く評価するため、法的根拠・購入方法等はAccordionから出す
- **記事間の重複回避**: how-to-use-altstore=「選び方」、altstore-complete-guide=「実践」と明確に棲み分け
