...
<Section title="サイドローディングのリスク" icon="warning" iconColor="text-red-600">
  <AccordionGroup>
    <Accordion title="ソース選びが重要">
      ...
      <InfoBox type="warning" icon="shield" title="安全なソースの見分け方">
        信頼できるリポジトリ以外からはインストールしないでください。GitHubでソースコードが公開されているアプリ（オープンソース）を選ぶのが一番安全です。
      </InfoBox>
    ...
    <Accordion title="7日ごとに署名が切れる">
      ...
      <InfoBox type="info" icon="wifi" title="自動更新で手間なし">
        AltStoreなら、PCと同じWi-Fiに接続しているだけで自動的に署名が更新されます。7日問題を意識する必要はほぼありません。
      </InfoBox>
    </Accordion>
  ...
  <div class="mt-4 p-4 bg-gradient-to-r from-pink-50 to-purple-50 border border-pink-200 rounded-xl">
    <div class="flex gap-3">
      ...
      <div class="flex-1 min-w-0">
        <div class="font-bold text-pink-700 text-sm mb-2">ぶんちゃんのアドバイス</div>
        <p class="text-sm text-gray-700 leading-relaxed">
          <strong>怖がりすぎなくてOK！</strong><br />
          AltStoreの公式ソースだけ使えば、安全性はかなり高いよ。「知らないサイトからIPAをダウンロード」だけは絶対やめてね⚠️
        </p>
      </div>
    </div>
  </div>
  ...
  <div class="mb-4 p-4 bg-gradient-to-r from-pink-50 to-purple-50 border border-pink-200 rounded-xl">
    <div class="flex gap-3">
      ...
      <div class="flex-1 min-w-0">
        <div class="font-bold text-pink-700 text-sm mb-2">ぶんちゃんのアドバイス</div>
        <p class="text-sm text-gray-700 leading-relaxed">
          <strong>100均のケーブルだと「充電専用」で認識しないことがあるよ。</strong><br />
          「MFi認証」って書いてあるケーブルを使うのが一番確実！最初のセットアップだけPC接続が必要で、あとはWi-Fiでいけるよ🌸
        </p>
      </div>
    </div>
  </div>
