...
<Section intro title="AltStoreとは何か" icon="help_outline">
  ...
  <InfoBox type="indigo" icon="auto_stories" title="この記事の位置づけ（仕組み・理論編）">
    ...
    実際のインストール手順は「<a href={`${import.meta.env.BASE_URL}articles/install-app-step-by-step`} class="font-bold underline">アプリをインストールする方法</a>」を、
    全体像は「<a href={`${import.meta.env.BASE_URL}articles/altstore-complete-guide`} class="font-bold underline">AltStore 完全ガイド</a>」をご覧ください。
  </InfoBox>
</Section>
