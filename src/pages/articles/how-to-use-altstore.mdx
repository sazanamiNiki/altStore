...
<ArticleLayout title={title} h1={h1} description={description} date="2026-03-03" icon="apps" iconColor="text-blue-500">
...
<Section intro title="AltStoreとは何か" icon="help_outline">
  <div class="space-y-3">
    ...
    <CardGrid>
      <Card icon="install_mobile" iconColor="text-blue-500" title="簡単インストール" desc="PCと接続して数タップでアプリ導入完了" />
      <Card icon="wifi" iconColor="text-green-500" title="Wi-Fi自動更新" desc="AltServerで7日署名を自動管理" />
      <Card icon="code" iconColor="text-purple-500" title="オープンソース" desc="コードが公開されていて安全性が検証可能" />
    </CardGrid>
    <div class="flex items-start gap-3 text-sm text-gray-700 leading-relaxed">
      ...
      <div class="space-y-1">
        <div class="font-semibold text-blue-700 text-sm">この記事の位置づけ（仕組み・理論編）</div>
    ...
    <Accordion title="Wi-Fi自動更新の条件">
      <InfoBox type="info" title="自動更新に必要な3つの条件">
        <ul class="mt-1 space-y-1">
          ...
          <li>iPhoneが<strong>ロック画面</strong>の状態（バックグラウンド更新）</li>
        </ul>
      </InfoBox>
    </Accordion>
...
<Section title="PCがない場合はどうする？" icon="help_center" iconColor="text-blue-600">
  <CardGrid>
    <Card icon="touch_app" iconColor="text-orange-500" title="手動更新" desc="7日ごとにAltStoreアプリで更新ボタン(↻)をタップ" />
    <Card icon="people" iconColor="text-blue-500" title="PCを借りる" desc="友人・家族のPCで1回AltServerを起動" />
    <Card icon="smartphone" iconColor="text-green-500" title="SideStoreに移行" desc="初回だけPCがあれば以降はiPhoneのみで完結" />
  </CardGrid>
  <AccordionGroup>
