...
<Section title="実際の導入手順（概要）" icon="integration_instructions" iconColor="text-blue-600">
  <StepList>
    <Step num={1} title="AltServerをインストール" desc="公式サイトからダウンロードして起動" image={`${import.meta.env.BASE_URL}assets/img/download.png`} alt="download" />
    <Step num={2} title="iPhoneにAltStoreのインストール" desc="iPhone を PC に接続。altServerで端末を選択" image={`${import.meta.env.BASE_URL}assets/img/device-select.png`} alt="select" />
    <Step num={3} title="Apple IDでログイン" desc="AltServer に Apple ID を入力" image={`${import.meta.env.BASE_URL}assets/img/login.png`} alt="select" />
    <Step num={4} title="IPhoneでアプリの許可" desc="設定 > 一般 > VPNとデバイス管理ので自身のIDの欄をタップ。詳細の信頼をタップ" image={`${import.meta.env.BASE_URL}assets/img/allow.png`} alt="allow"/>
    <Step num={5} title="IPhoneでアプリの許可" desc="Finder で「 WI-FI経由でIPhoneとの接続 」を有効にすれば接続が不要になります。" image={`${import.meta.env.BASE_URL}assets/img/connect.png`} alt="connect"/>
  </StepList>
