...
<Section intro title="AltStoreを使いこなすための完全ガイド" icon="lightbulb">
  ...
  <InfoBox type="info" icon="auto_stories" title="どの方法を選ぶべきかわからない？">
    PAL・Classic・SideStoreの違いが分からない方は、まず「<a href={`${import.meta.env.BASE_URL}articles/how-to-use-altstore`} class="font-bold underline">AltStore選び方ガイド</a>」で自分に合った方法を確認してください。このページはセットアップ・運用の実践ガイドです。
  </InfoBox>
</Section>
...
<Section title="このガイドの対象" icon="compare" iconColor="text-blue-600">
  <p class="text-gray-700 leading-relaxed sm:leading-7 mb-3 sm:mb-4">
    このガイドでは主に<strong>AltStore Classic</strong>のセットアップ・運用を解説します。AltStore PALは基本的にSafariからインストールするだけで完了するため、以下のセクション（7日署名・AltServer・トラブル対応）は<strong>Classic利用者向け</strong>の内容です。
  </p>
  <CardGrid>
    <Card icon="storefront" iconColor="text-green-500" title="AltStore PAL" desc="Safariで入れるだけ（解説不要）" />
    <Card icon="apps" iconColor="text-orange-500" title="Classic（このガイド）" desc="セットアップ→運用→トラブル対応" />
  </CardGrid>
  <InfoBox type="info" icon="route" title="PAL・Classic・SideStoreの違いは？">
    「<a href={`${import.meta.env.BASE_URL}articles/how-to-use-altstore`} class="font-bold underline">AltStore選び方ガイド</a>」で3つの方法を比較しています。どれを使うか決まっていない方はそちらを先にご覧ください。
  </InfoBox>
</Section>
...
<RelatedArticles articles={[
  {
    ...
    title: 'PAL・Classic・SideStore選び方ガイド',
    desc: '3つの方法を比較して、自分に合ったサイドローディング方法を選ぶ。',
  },
  {
