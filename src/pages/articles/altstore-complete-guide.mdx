...
<ArticleLayout title={title} h1={h1} description={description} date="2026-02-10" icon="apps" iconColor="text-blue-500">
...
<Section intro title="AltStoreを使いこなすための完全ガイド" icon="lightbulb">
  ...
  <InfoBox type="indigo" icon="auto_stories" title="より詳しく学ぶための関連記事">
    ...
    より詳しく学びたい方は、
    「<a href="/articles/how-to-use-altstore" class="font-bold underline">AltStore の仕組みと導入の基本（理論編）</a>」や
    「<a href="/articles/install-app-step-by-step" class="font-bold underline">アプリをインストールする方法（実践編）</a>」もご覧ください。
  </InfoBox>
</Section>
...
<Section title="初期セットアップ手順" icon="construction" iconColor="text-blue-600">
  <StepList>
    <Step num={1} title="AltServerをダウンロード＆インストール" desc="公式サイトからPC/Mac用のAltServerをダウンロードしてインストール。Macの手順・Windowsの手順が公式FAQに掲載されています（altstore.io）。" />
    <Step num={2} title="AltServerにApple IDでログイン" desc="AltServerを起動し、メニューから「Sign In」を選択。iPhoneで使用しているApple IDでサインイン。2段階認証コードの入力が必要な場合があります。" />
    <Step num={3} title="iPhoneをUSBで接続してAltStoreをインストール" desc="iPhoneをUSBケーブルでPC/Macに接続 → 「このコンピューターを信頼しますか？」→「信頼」をタップ → AltServerメニューから「Install AltStore」を選択 → 数秒でiPhoneのホーム画面にAltStoreが追加される。iPhone上でAltStoreを起動し、Apple IDでサインイン。" />
    <Step num={4} title="Wi-Fi経由での自動更新設定（推奨）" desc="PC/MacのAltServerメニューからインストール済みのiPhoneを選択 → 「Sync with [iPhone name] over Wi-Fi」オプションをオン。PC/MacとiPhoneが同じWi-Fiネットワークに接続し、AltServerをバックグラウンドで起動したままにすると自動更新が機能します。" />
  </StepList>
  <InfoBox type="warning" icon="info" title="自動更新と公証ステータスについて">
    AltServer による自動更新では、7日ごとに再署名が行われます。
    この再署名プロセスでは<strong>Apple 公証ステータスが毎回失われます</strong>が、
    iOS のサンドボックス構造により基本的な安全性は維持されます。
    詳しくは「<a href="/articles/apple-notarization" class="font-bold underline">Apple の公証とは何か</a>」をご覧ください。
  </InfoBox>
</Section>
  ...
  <AccordionGroup>
    <Accordion title="方法1：AltStore内のアプリ一覧から">
      <ol class="list-decimal pl-6 space-y-2 text-gray-700">
        ...
        <li>インストールしたいアプリをタップして詳細ページへ</li>
        <li>「Get」ボタンをタップ</li>
    ...
    <Accordion title="方法2：IPAファイルをiPhoneで直接インストール">
      ...
      <ol class="list-decimal pl-6 space-y-2 text-gray-700 mb-3">
        <li>信頼できるソースから IPA ファイルをダウンロード（「ファイル」アプリに保存）</li>
        <li>AltStore アプリを起動</li>
        <li>下部タブの「My Apps」をタップ</li>
        <li>左上の「+」アイコンをタップ</li>
        <li>「ファイル」からダウンロードした IPA ファイルを選択</li>
        <li>インストールが始まり、完了するとホーム画面にアプリアイコンが表示される</li>
      </ol>
    ...
    <Accordion title="方法3：AltServerからIPAをサイドロード（PC使用）">
      ...
      <ol class="list-decimal pl-6 space-y-2 text-gray-700">
        <li>信頼できるソースから IPA ファイルを PC/Mac にダウンロード</li>
        <li>AltServer メニューを開き「Sideload .ipa」などを選択</li>
        <li>IPA ファイルを指定してインストール</li>
      </ol>
    </Accordion>
  </AccordionGroup>
...
<Section title="トラブルシューティング" icon="help" iconColor="text-blue-600">
  <AccordionGroup>
    ...
    <Accordion title="インストール後、アプリが起動しない">
      <p class="text-gray-700 mb-3"><strong>原因：</strong>署名が切れている可能性があります。</p>
      <ol class="list-decimal pl-6 space-y-2 text-gray-700">
        <li>AltStore アプリを開く</li>
        <li>該当アプリの横の更新ボタン（↻）をタップ</li>
        <li>署名を更新（通常、数分で起動可能になります）</li>
      </ol>
    </Accordion>
    ...
    <Accordion title="複数のiPhoneで使える？">
      <p class="text-gray-700 mb-3">
        可能です。同じ Apple ID で複数の端末を登録すれば、複数の iPhone/iPad で AltStore を使用できます。
      </p>
    ...
    <Accordion title="PCがない場合はどうする？">
      <p class="text-gray-700 mb-3">
        家族や友人の PC/Mac を借りて初回セットアップを完了させれば、
        その後は iPhone 上で手動で署名を更新できます。毎回 USB 接続は不要です。
      </p>
      <p class="text-gray-700">
    ...
    <Accordion title="Apple IDが複数ある場合は？">
      <p class="text-gray-700">
        AltServer と AltStore に<strong>同じ Apple ID</strong> でサインインしてください。
